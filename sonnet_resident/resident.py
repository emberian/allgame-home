...
from sonnet_resident.sessions import SessionManager
from sonnet_resident.judge import EngagementJudge
from sonnet_resident.reactions import emoji_display, handle_reaction_event
from sonnet_resident.prompt import (
    SYSTEM_PROMPT,
...
from sonnet_resident.images import build_content_blocks
from sonnet_resident.archive import archive_api_response
from sonnet_resident.sandbox import cleanup_sandbox
...
from sonnet_resident.tools.comms_tools import write_sysadmin_message
...
class ClaudeResident:
    ...
    def __init__(self, zulip_client, anthropic_client,
        ...
        self.logger = logging.getLogger("sonnet_resident")
    ...
    def _generate_response_with_tools_session(
        ...
        # Sonnet uses explicit-budget thinking (adaptive is opus-only).
        ...
        try:
            for turn in range(MAX_TOOL_TURNS):
                ...
                # Cost: Sonnet 4.6 pricing per MTok
                # input $3, cache_write_5m $3.75, cache_read $0.30, output $15
                cost_input = input_tokens * 3.0 / 1_000_000
                cost_cache_write = cache_create * 3.75 / 1_000_000
...
Include <state_update> blocks if you want to remember anything."""
    ...
    def arrive(self, stream: str = "clankerville",
               topic: str = "tavern"):
...
about this moment."""
        ...
        # The regular system prompt tells the model "text is invisible, you
        # must call send_message." So arrive() must pass tools and look for
        # send_message tool_use blocks, not harvest response.content[0].text.
        try:
            response = self.anthropic.messages.create(
                model=self.model,
                max_tokens=MAX_RESPONSE_TOKENS,
        ...
        for block in response.content:
            if block.type == "tool_use" and block.name == "send_message":
                ...
                if not content:
                    continue
                target_stream = inp.get("stream", stream)
                target_topic = inp.get("topic", topic)
                result = self.zulip.send_message({
                    "type": "stream",
                    "to": target_stream,
                    "topic": target_topic,
                ...
                if result.get("result") == "success":
                    posted = True
                    self.logger.info(
                        f"Arrived in #{target_stream}>{target_topic}")
                else:
                    self.logger.error(
                        f"Failed to post arrival: {result}")
            elif block.type == "text":
                fallback_text_parts.append(block.text)

        # Fallback: if the model produced text but no send_message tool_use,
        # strip state_updates / sysadmin blocks and post the remaining text.
        if not posted and fallback_text_parts:
            response_text = "".join(fallback_text_parts)
            clean_response, state_updates = _extract_state_updates(
                response_text)
            clean_response, sysadmin_messages = _extract_sysadmin_messages(
                clean_response)
            ...
            if clean_response.strip():
                self.logger.warning(
                    "Arrival: model emitted text without send_message; "
                    "posting fallback text.")
                result = self.zulip.send_message({
                    "type": "stream",
                    "to": stream,
                    "topic": topic,
                ...
                if result.get("result") == "success":
                    self.logger.info(
                        f"Arrived (fallback) in #{stream}>{topic}")
                else:
                    self.logger.error(
                        f"Failed to post arrival fallback: {result}")
